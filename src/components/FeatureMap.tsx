  ...
  return (
    <div className="w-full text-center">
      <svg
        ...
        className={`mx-auto block rounded border border-stone-200 ${internal ? "w-full max-w-32" : spectogram ? "w-full object-contain" : "max-h-[300px] w-full max-w-[500px] object-contain"}`}
      >
        {data.flatMap((row, i) =>
          row.map((value, j) => {
      ...
      <p className="text-x5 mt-1 text-stone-500">{title}</p>
    </div>
  );
};
